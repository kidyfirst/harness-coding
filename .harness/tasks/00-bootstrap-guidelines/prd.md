...
`.harness/` now exists with empty spec scaffolding, and this bootstrap task
exists under `.harness/tasks/`. When they want to work on it, they should start
this task from a session that provides Harness session identity.

**Your job**: help them populate `.harness/spec/` with the team's real
...
- [ ] Fill guidelines for harness-spec
- [ ] Add code examples
...
### Package: harness-spec (`spec/harness-spec/`)

- Backend guidelines: `.harness/spec/harness-spec/backend/`

- Frontend guidelines: `.harness/spec/harness-spec/frontend/`


### Thinking guides (already populated)

`.harness/spec/guides/` contains general thinking guides pre-filled with
...
extract the relevant rules into the matching `.harness/spec/` files —
...
- Source of truth: `.harness/spec/`. That's why filling it well now pays
  off forever.
...
node ./.harness/scripts/task.js archive 00-bootstrap-guidelines
```
