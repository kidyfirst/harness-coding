...
```bash
TASK_DIR=$(node ./.${your-name}/scripts/task.js create "brainstorm: <short goal>" --slug <auto>)
```
...
```bash
# Create child tasks
CHILD1=$(node ./.${your-name}/scripts/task.js create "Child task 1" --slug child1 --parent "$TASK_DIR")
CHILD2=$(node ./.${your-name}/scripts/task.js create "Child task 2" --slug child2 --parent "$TASK_DIR")

# Or link existing tasks
node ./.${your-name}/scripts/task.js add-subtask "$TASK_DIR" "$CHILD_DIR"
```
