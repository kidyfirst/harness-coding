...
`spec/` is the project's engineering spec library. The goal is not to make AI memorize conventions forever; it is to inject relevant specs or require the AI to read them at the right time.
...
`config.yaml` can declare packages:
...
```jsonl
{"file": ".${your-name}/spec/cli/backend/index.md", "reason": "CLI backend conventions"}
{"file": ".${your-name}/spec/cli/unit-test/conventions.md", "reason": "Test expectations"}
```
...
When the AI learns a new rule during implementation or debugging, it should update `spec/` rather than only summarizing it in chat.

## Local Customization Points

| Need | Edit location |
| --- | --- |
| Add a new spec layer | `spec/<package>/<layer>/index.md` and corresponding guideline files. |
| Change monorepo spec mapping | `packages`, `default_package`, or `spec_scope` in `config.yaml`. |
...
| Change when specs should be updated | Phase 3.3 in `workflow.md` and the local spec-update entry point. |

## Boundaries

`spec/` is the project's specification, not a permanent copy of built-in templates. The AI should encourage the user to update it according to the actual project code instead of treating defaults as immutable documents.
