...
`workflow.md` is the workflow source of truth inside the user project. An AI does not need CLI source code to understand how the current project should move tasks forward; this file is enough.

All paths below are relative to the personalized workflow root, for example `.${your-name}/`.

## File Responsibilities

`workflow.md` has three responsibilities:
...
- Platforms with sub-agent support: dispatch implement/check agents by default.
- Platforms without sub-agent support: the main session reads workflow skills and executes directly.
...
| Change the wrap-up flow | Update Phase 3 and finish-related descriptions. |
| Change platform differences | Update routing descriptions grouped by platform. |

After editing, make the AI reread `workflow.md`; do not assume the flow from the old conversation is still valid.
