...
description: "Bootstrap project-specific Harness Spec coding specs with a platform-neutral single-agent workflow. Use when creating or refreshing .${your-name}/spec guidelines, analyzing a codebase with GitNexus, ABCoder, or source inspection, decomposing package/layer spec work, and writing real codebase-backed spec docs without placeholder text."
---

# Harness Spec Bootstrap

Use this skill to create or refresh `.${your-name}/spec/` guidelines from the real codebase. One capable agent owns the full loop: analyze the repository, choose the spec boundaries, write the docs, and verify the result. The workflow does not depend on a specific host, CLI, or agent brand.

## Workflow

1. Confirm Harness Spec is initialized and inspect the current `.${your-name}/spec/` tree.
...
| Writing high-signal Harness Spec spec files | [references/spec-writing.md](references/spec-writing.md) |
...
- Do not leave placeholder text, empty headings, or copied boilerplate in `.${your-name}/spec/`.

## Done Criteria

- `.${your-name}/spec/` describes the project as it exists now.
