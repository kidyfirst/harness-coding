...
Use a single agent as the default execution model. The agent may create Harness Spec tasks for traceability, but the skill should not require a specific platform, CLI, or parallel worker model.
...
When a Harness Spec task is useful, write a concise PRD with these sections:

```markdown
# Fill <package-or-layer> Harness Spec Docs

## Goal
Write project-specific `.${your-name}/spec/` guidance for <scope>.
...
- `.${your-name}/spec/.../index.md`
- `.${your-name}/spec/.../<topic>.md`
