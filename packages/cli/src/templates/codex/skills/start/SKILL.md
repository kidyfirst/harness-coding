---
name: start
description: "Initializes an AI development session by reading workflow guides, project owner record, git status, active tasks, and project guidelines from .${your-name}/. Classifies incoming tasks and routes to brainstorm, direct edit, or task workflow. Use when beginning a new coding session, resuming work, starting a new task, or re-establishing project context."
---
...
This shows: project owner record, git status, current task (if any), active tasks.
...
```bash
cat .${your-name}/spec/<package>/<layer>/index.md   # Package-specific guidelines
cat .${your-name}/spec/guides/index.md              # Thinking guides (always read)
```
...
```bash
TASK_DIR=$(node ./.${your-name}/scripts/task.js create "<title>" --slug <name>)
```
...
1. Relevant spec files in `.${your-name}/spec/`
...
`implement.jsonl` and `check.jsonl` were seeded on `task.js create` with a single self-describing `_example` line. Curate real entries now (see workflow.md Phase 1.3 for the full rule):

- Put **spec files** (`.${your-name}/spec/<package>/<layer>/*.md`) and **research files** (`{TASK_DIR}/research/*.md`) only.
...
```bash
node ./.${your-name}/scripts/task.js add-context "$TASK_DIR" implement "<path>" "<reason>"
node ./.${your-name}/scripts/task.js add-context "$TASK_DIR" check "<path>" "<reason>"
```

**Step 7: Activate Task** `[AI]`

```bash
node ./.${your-name}/scripts/task.js start "$TASK_DIR"
```

This sets the active task through Harness Spec's session resolver so hooks can inject context for this AI session. If the command fails because no session identity is available, rerun it from an IDE/session that exposes session identity or set `HARNESS_SPEC_CONTEXT_ID`.
...
If `get_context.js` shows a current task:
...
| Script | Purpose |
|--------|---------|
| `node ./.${your-name}/scripts/get_context.js` | Get session context |
| `node ./.${your-name}/scripts/task.js create` | Create task directory (seeds jsonl on sub-agent platforms) |
| `node ./.${your-name}/scripts/task.js add-context` | Append spec/research entry to jsonl |
| `node ./.${your-name}/scripts/task.js start` | Set current task |
| `node ./.${your-name}/scripts/task.js finish` | Clear current task |
| `node ./.${your-name}/scripts/task.js archive` | Archive completed task |
