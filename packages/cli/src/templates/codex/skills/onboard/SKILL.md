---
name: onboard
description: "Interactive three-part onboarding for new team members to the Harness Spec AI-assisted workflow system. Covers core philosophy (AI memory, project-specific knowledge, context drift), system structure and command deep-dives, real-world workflow examples, and guideline customization. Use when a new project owner joins the project, someone needs to understand the Harness Spec workflow, or project guidelines need initial setup."
---

You are a senior engineer onboarding a new teammate to this project's AI-assisted workflow system.
...
- If empty, guide the project owner to fill them with project-specific content
...
After completing ALL THREE parts, ask the project owner about their first task.
...
**The Solution**: The `.${your-name}/workspace/` system captures what happened in each session - what was done, what was learned, what problems were solved. The `$start` skill reads this history at session start, giving AI "artificial memory."
...
**The Solution**: The `.${your-name}/spec/` directory contains project-specific guidelines. The `$before-*-dev` skills inject this specialized knowledge into AI context before coding starts.
...
```
.${your-name}/
|-- .project-owner              # Project owner record (gitignored)
|-- workflow.md             # Complete workflow documentation
|-- workspace/              # "AI Memory" - session history
|   |-- index.md            # All project-owner activity
|   +-- {project-owner}/    # Project-owner directory
|       |-- index.md        # Project-owner progress index
...
1. Reads project owner record (who is the current project owner in this checkout?)
...
**WHAT IT ACTUALLY DOES**:
1. Discovers spec layers via `get_context.js --mode packages` and reads relevant guidelines
...
**WHAT IT ACTUALLY DOES**:
1. Records session summary to `workspace/{project-owner}/journal-N.md`
...
**[2/8] node ./.${your-name}/scripts/task.js create "Fix bug" --slug fix-bug** - Track work for future reference
...
**[2/4] node ./.${your-name}/scripts/task.js create "Planning task" --slug planning-task** - Planning is valuable work
...
Check if `.${your-name}/spec/` contains empty templates or customized guidelines:

```bash
# Check if files are still empty templates (look for placeholder text)
grep -l "To be filled by the team" .${your-name}/spec/backend/*.md 2>/dev/null | wc -l
grep -l "To be filled by the team" .${your-name}/spec/frontend/*.md 2>/dev/null | wc -l
```
...
If guidelines are empty templates (contain "To be filled by the team"), this is the first time using Harness Spec in this project.

Explain to the project owner:

"I see that the development guidelines in `.${your-name}/spec/` are still empty templates. This is normal for a new Harness Spec setup!
...
For example, for `.${your-name}/spec/backend/database-guidelines.md`:
...
I recommend reading through `.${your-name}/spec/` to familiarize yourself with the team's coding standards."
...
```bash
node ./.${your-name}/scripts/task.js create "Fill spec guidelines" --slug fill-spec-guidelines
```
...
"You're now onboarded to the Harness Spec workflow system! Here's what we covered:
...
2. [If guidelines empty] Start filling in `.${your-name}/spec/` guidelines
