...
description: "Adapts an external skill into project-specific development guidelines in the personalized workflow spec tree. Creates guideline sections, code example templates with .template suffix, and updates spec indexes. Use when integrating an external skill, adding a new skill's patterns to project conventions, or incorporating third-party skill best practices into local workflow documentation."
---
...
>
> - Guidelines content -> Write to `.${your-name}/spec/{target}/doc.md`
> - Code examples -> Place in `.${your-name}/spec/{target}/examples/skills/<skill-name>/`
...
|----------------|-------------------|
| UI/Frontend (`frontend-design`, `web-artifacts-builder`) | `.${your-name}/spec/frontend/` |
| Backend/API (`mcp-builder`) | `.${your-name}/spec/backend/` |
| Documentation (`doc-coauthoring`, `docx`, `pdf`) | `.${your-name}/` or create dedicated guidelines |
| Testing (`webapp-testing`) | `.${your-name}/spec/frontend/` (E2E) |
...
- **Integration target**: `.${your-name}/spec/{target}/`
...
| Type | Path |
|------|------|
| Guidelines doc | `.${your-name}/spec/{target}/doc.md` (section: `skill-<name>`) |
| Code examples | `.${your-name}/spec/{target}/examples/skills/<name>/` |
| Index update | `.${your-name}/spec/{target}/index.md` |
...
| `doc-coauthoring` | `.${your-name}/` | N/A (documentation workflow only) |
