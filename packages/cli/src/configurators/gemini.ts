 ...
 * - commands/harness-spec/ — start + finish-work as TOML slash commands
 * - .agents/skills/harness-{name}/SKILL.md — 5 auto-triggered shared skills
...
export async function configureGemini(
  ...
  const commandsDir = path.join(configRoot, "commands", "harness-spec");
  ...
  await writeFile(
    ...
    resolvePlaceholders(getSettingsTemplate(language)),
  );

  removeDirIfEmpty(path.join(configRoot, "skills"));
}
