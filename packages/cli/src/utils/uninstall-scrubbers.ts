/**
 * Scrubbers for structured config files during `harness-spec uninstall`.
 *
 ...
 * - `fullyEmpty` is true when, after stripping every harness-spec-managed value,
 ...
 *
 * Harness Spec-emitted hook commands always have the shape
    ...
    for (const entry of eventArr) {
      if (mode === "flat") {
        ...
        if (cmd !== null && commandMatchesDeletedPath(cmd, deletedPaths)) {
          continue; // drop harness-spec entry
        }
...
/**
 * Harness Spec-specific values written by the Pi configurator.
 *
 * The `extensions`/`skills`/`prompts` arrays are paths relative to `.pi/`. We
 * remove the exact entries that the Pi configurator emits.
 */
const PI_HARNESS_EXTENSION = "./extensions/harness-spec/index.ts";
const PI_HARNESS_SKILLS = "./skills";
const PI_HARNESS_PROMPTS = "./prompts";
const PI_SUBAGENTS_PACKAGE = "npm:pi-subagents";

function isHarnessPiEntry(value: unknown, target: string): boolean {
  return typeof value === "string" && value === target;
}

/**
 * Scrub `.pi/settings.json`:
 * - drop `enableSkillCommands` (harness-spec-flagged)
 * - remove harness-spec entries from `extensions`/`skills`/`prompts` arrays
 * - remove harness-spec-managed `packages["npm:pi-subagents"]` isolation override
...
export function scrubPiSettings(content: string): ScrubResult {
  ...
  const arrayCleanups: [string, string][] = [
    ["extensions", PI_HARNESS_EXTENSION],
    ["skills", PI_HARNESS_SKILLS],
    ["prompts", PI_HARNESS_PROMPTS],
  ];
  for (const [key, target] of arrayCleanups) {
    const arr = root[key];
    if (!Array.isArray(arr)) continue;
    const filtered = arr.filter((v) => !isHarnessPiEntry(v, target));
 ...
 *
 * The current harness-spec-emitted file has two distinct chunks:
 ...
 *  - any line that is *only* a comment introduced by harness-spec (the entire file
 *    as shipped is comments + that one assignment)
 *  - blank lines that surrounded those removals
 *
 * If the user added their own non-harness-spec lines, they are preserved as-is.
...
export function scrubCodexConfigToml(content: string): ScrubResult {
  const harnessCommentMarkers = [
    "Project-scoped Codex defaults for Harness Spec workflows.",
  ...
  // A comment line is "known managed text" if its content (after `#` and
  // spaces) matches one of the known marker strings exactly OR is an empty `#` line.
  function isManagedCommentLine(line: string): boolean {
    const trimmed = line.trim();
    if (!trimmed.startsWith("#")) return false;
    const inner = trimmed.replace(/^#+\s?/, "").trim();
    if (inner.length === 0) return true; // bare `#` line inside managed block
    return harnessCommentMarkers.some((m) => inner === m);
  }

  function isManagedAssignment(line: string): boolean {
  ...
  for (const rawLine of content.split(/\r?\n/)) {
    if (isManagedAssignment(rawLine) || isManagedCommentLine(rawLine)) {
      continue; // drop
    }
