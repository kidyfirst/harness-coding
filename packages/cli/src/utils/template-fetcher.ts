/**
 * Remote template fetcher for Harness Spec CLI
 *
...
export async function fetchTemplateIndex(
  ...
  try {
    ...
    const res = await fetch(url, {
      signal: createTimeoutSignal(TIMEOUTS.INDEX_FETCH_MS),
    });
    if (!res.ok) {
...
async function cloneRegistryRef(
  ...
  const dir = await fs.promises.mkdtemp(
    path.join(os.tmpdir(), "harness-registry-"),
  );
  try {
    try {
      await runGit([
...
export async function downloadWithStrategy(
  ...
  if (strategy === "append" && exists) {
    const tempDir = path.join(os.tmpdir(), `harness-template-${Date.now()}`);
    try {
      await withTimeout(
