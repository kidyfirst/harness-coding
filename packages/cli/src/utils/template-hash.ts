...
function getHashesPath(cwd: string): string {
  return path.join(cwd, getConfiguredWorkflowRoot(cwd), HASHES_FILE);
}
...
/**
 * Patterns to exclude from hash tracking for the workflow tree walk.
 */
const EXCLUDE_FROM_HASH = [
  ".template-hashes.json", // Hash file itself
  ".version", // Version file
  ".gitignore", // Git ignore files
  ".project-owner", // Project owner record file
...
export interface InitializeHashesOptions {
  /**
   * POSIX-style relative paths written during the init run
 ...
 *
 * The personalized workflow root is still walked recursively (with
 * `EXCLUDE_FROM_HASH`) because uninstall removes that tree wholesale
 * regardless of manifest content. Accuracy there only affects update-time
 * 3-way merge fidelity.
 *
...
export function initializeHashes(
  ...
  if (trackedPaths) {
    ...
    for (const relativePath of trackedPaths) {
      // Workflow-root paths are handled by the walk below — don't double-track.
      if (
        relativePath.startsWith(`${workflowRoot}/`) ||
        relativePath === workflowRoot
      ) {
        continue;
      }
  ...
  // Workflow tree: still walked recursively. Accuracy here is for update's
  // 3-way merge of workflow.md / config.yaml / scripts; uninstall removes the
  // whole workflow root, so it does not matter for the data-loss bug this
  // contract addresses.
  const workflowRoot = getConfiguredWorkflowRoot(cwd);
  const files = collectFiles(cwd, workflowRoot);
