...
// Write recording
//
// `harness-spec init` uses recording to capture exactly which files were actually
...
export async function writeFile(
  ...
  if (existingContent === content) {
    ...
    // byte-identical to a managed template and still not be harness-spec-owned.
    return false;
  }
  ...
  if (mode === "skip") {
    ...
    // Skipped: harness-spec did NOT write this file — caller should not track it
  ...
  if (mode === "append") {
    ...
    // Append: harness-spec added managed content to a user-owned file. Tracking
