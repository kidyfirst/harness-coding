 ...
 *
 * Running `harness-spec init` / `harness-spec uninstall` in `$HOME` is catastrophic:
...
/**
 * Error message printed by both `harness-spec init` and `harness-spec uninstall` when
 * the homedir guard trips.
 */
export function homedirGuardMessage(commandName: "init" | "uninstall"): string {
  return (
    `✗ Refusing to run \`harness-spec ${commandName}\` in your home directory.\n\n` +
    `Harness Spec manages platform config dirs like .claude/, .codex/, .opencode/, which\n` +
    ...
    `Run harness-spec from your project directory instead. If you really want to run in\n` +
    `$HOME, set HARNESS_ALLOW_HOMEDIR=1.`
  );
}
...
export function homedirBypassEnabled(): boolean {
  return process.env.HARNESS_ALLOW_HOMEDIR === "1";
}
