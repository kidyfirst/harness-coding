 ...
 * `harness-spec update` (before migration classification) and `harness-spec uninstall`
 * (before plan building). Together they ensure existing poisoned manifests
 * self-correct on the next routine command.
 *
 * Rules:
 *   - The current personalized workflow-root entries are always kept.
 *   - Root-level `AGENTS.md` is kept only when it still looks harness-managed
 ...
 *     pruned. This matches "files harness-spec actually wrote during init/update".
 */
...
import { getConfiguredWorkflowRoot } from "./workflow-root.js";
...
/**
 * Compute the union of "what harness-spec writes" across:
 *   - every configured platform's collectTemplates() output
 *   - root-level AGENTS.md when it still carries managed-block markers
...
export function pruneOrphanManifestKeys(
  ...
  const workflowRoot = toPosix(getConfiguredWorkflowRoot(cwd));
  const pruned: string[] = [];
  const kept: TemplateHashes = {};

  for (const [rawKey, value] of Object.entries(hashes)) {
    const key = toPosix(rawKey);
    if (key === workflowRoot || key.startsWith(`${workflowRoot}/`)) {
      kept[key] = value;
      continue;
    }
