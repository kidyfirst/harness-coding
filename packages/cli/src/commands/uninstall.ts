/**
 * `harness-spec uninstall` — remove every file written by `harness-spec init` / `update`
 * from the current project, plus the workflow directory itself.
 *
 * The single source of truth for managed files is
 * `WORKFLOW_ROOT/.template-hashes.json`. Files outside that manifest are never
 ...
 *      package.json) — passed through a scrubber that strips just the managed
 ...
 * The workflow tree is removed unconditionally.
 */
...
interface StructuredFileSpec {
   ...
   * harness-spec-managed `command` strings.
   */
  ...
  const specs: StructuredFileSpec[] = [
    ...
    ...(
      ...
      (p): StructuredFileSpec => ({
        posixPath: p,
        reason: "Strip Harness Spec hooks; preserve user fields",
    ...
    },
    {
      ...
      reason:
        "Strip Harness Spec extension/skills/prompts entries; preserve user fields",
      scrub: (content) => scrubPiSettings(content),
    },
    {
      posixPath: ".codex/config.toml",
      reason: "Remove Harness Spec project_doc_fallback_filenames and notes",
...
interface UninstallPlan {
  ...
  /** Whether the workflow root directory itself will be removed. */
  removeWorkflowDir: boolean;
}
...
function renderPlan(cwd: string, plan: UninstallPlan): void {
  const workflowRoot = getConfiguredWorkflowRoot(cwd);
  const workflowDirPath = path.join(cwd, workflowRoot);

  console.log(chalk.bold("\nHarness Spec uninstall plan\n"));
  ...
  if (plan.removeWorkflowDir && fs.existsSync(workflowDirPath)) {
    console.log(
      `  ${chalk.red("-")} ${workflowRoot}/  ${chalk.gray(
...
/**
 * Execute the plan: write modifications, unlink deletions, remove the
...
function executePlan(
  ...
  // 3. Drop the workflow root entirely.
  let deletedDirs = 0;
  if (plan.removeWorkflowDir) {
    const workflowRoot = getConfiguredWorkflowRoot(cwd);
    const workflowDirPath = path.join(cwd, workflowRoot);
  ...
  // has no purpose. The workflow root is already gone (step 3), so we skip it.
...
export async function uninstall(options: UninstallOptions = {}): Promise<void> {
  ...
  // Pre-check 1: must have the personalized workflow root directory.
  if (!fs.existsSync(workflowDirPath)) {
    console.log(
      chalk.gray(
        `Harness Spec is not installed in this project (no ${workflowRoot}/ directory found).`,
      ),
    );
    return;
  }
  ...
  // platform files are harness-spec-owned vs user-owned.
  const hashes = loadHashes(cwd);
  if (Object.keys(hashes).length === 0) {
    console.error(
      chalk.red(
        "Harness Spec workflow directory found but manifest is missing — cannot determine which platform files to remove. " +
          `You can manually delete ${workflowRoot}/ if needed.`,
      ),
    );
    process.exit(1);
  }
  ...
  // manifest only to delete the whole workflow root anyway — but the
  ...
  if (pruned.length > 0) {
    ...
    console.log(
      chalk.gray(
        `   Pruned ${pruned.length} orphan manifest entries (user-owned files harness-spec did not write).`,
      ),
    );
  }
  ...
  if (!options.yes) {
    ...
    // honor the same UX as `harness-spec update` (which also fails closed in
    // that case).
  ...
  console.log(
    chalk.green(
      `Uninstalled harness-spec: ${summary.deletedFiles} files deleted, ` +
