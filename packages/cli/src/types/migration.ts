/**
 * Shared manifest/hash typing for the Harness Spec update command.
 *
 * The current release line no longer performs legacy project migration, but
 * the manifest and hash file formats still benefit from shared types.
 */
 ...
 *
 * Used by `harness-spec update` to append the section to existing user config files
...
export interface ConfigSectionAdded {
  /** Target file relative to project root (e.g. `WORKFLOW_ROOT/config.yaml`). */
  file: string;
  /**
...
export interface MigrationManifest {
  ...
  breaking?: boolean;
  /** Whether the release recommends extra manual review before updating */
