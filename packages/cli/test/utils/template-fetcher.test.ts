...
describe("getInstallPath", () => {
  it("returns spec path for 'spec' type", () => {
    ...
    expect(result).toBe(path.join("/project", "WORKFLOW_ROOT/spec"));
  });
  ...
  it("works with different cwd values", () => {
    ...
    expect(result).toBe(path.join("/home/user/my-project", "WORKFLOW_ROOT/spec"));
  });
});
...
async function withGitRegistry<T>(
  ...
  const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "harness-git-reg-"));
...
async function withFakeGit<T>(
  ...
  const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "harness-fake-git-"));
  ...
  it("downloads marketplace templates through the same Git backend", async () => {
    await withGitRegistry(
      ...
      async ({ registry, tmpDir }) => {
        ...
        expect(
          fs.readFileSync(
            path.join(cwd, "WORKFLOW_ROOT", "spec", "rules.md"),
            "utf-8",
          ),
        ).toBe("remote rules\n");
      },
  ...
  it("uses direct download mode when index.json is absent", async () => {
    await withGitRegistry(
      ...
      async ({ registry, tmpDir }) => {
        ...
        const cwd = path.join(tmpDir, "project");
        const specDir = path.join(cwd, "WORKFLOW_ROOT", "spec");
