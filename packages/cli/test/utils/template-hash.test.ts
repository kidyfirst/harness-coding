...
import { getConfiguredWorkflowRoot } from "../../src/utils/workflow-root.js";

const UNSUPPORTED_WORKFLOW_ROOT = ".legacy-workflow";
const HASH_TEST_TMP_PREFIX = "harness-hash-test-";
const RUNTIME_TASK_SCRIPT = "task.js";
const UNSUPPORTED_CONFIG_PATH = `${UNSUPPORTED_WORKFLOW_ROOT}/config.yaml`;
const UNSUPPORTED_SCRIPTS_DIR = path.join(UNSUPPORTED_WORKFLOW_ROOT, "scripts");
const PERSONALIZED_WORKFLOW_ROOT = ".alice";
const PERSONALIZED_SCRIPTS_DIR = path.join(
  PERSONALIZED_WORKFLOW_ROOT,
  "scripts",
);

function ensureHashesRoot(tmpDir: string): void {
  fs.writeFileSync(
  ...
  fs.mkdirSync(path.join(tmpDir, PERSONALIZED_WORKFLOW_ROOT), { recursive: true });
}

function getCurrentWorkflowRoot(tmpDir: string): string {
  return getConfiguredWorkflowRoot(tmpDir);
}
...
describe("loadHashes / saveHashes", () => {
  ...
  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), HASH_TEST_TMP_PREFIX));
    fs.mkdirSync(path.join(tmpDir, UNSUPPORTED_WORKFLOW_ROOT), { recursive: true });
    ensureHashesRoot(tmpDir);
  });
  ...
  it("loadHashes returns empty object for invalid JSON", () => {
    const hashesPath = path.join(tmpDir, PERSONALIZED_WORKFLOW_ROOT, ".template-hashes.json");
...
describe("initializeHashes", () => {
  ...
  it("hashes files in the personalized workflow root and tracked platform paths", () => {
    // The personalized workflow root is always walked recursively. Platform paths (.claude/, etc.)
    ...
    fs.mkdirSync(path.join(tmpDir, PERSONALIZED_SCRIPTS_DIR), { recursive: true });
    fs.writeFileSync(
      path.join(tmpDir, PERSONALIZED_SCRIPTS_DIR, RUNTIME_TASK_SCRIPT),
      "console.log('hello')",
    );
    ...
    expect(hashes).toHaveProperty(
      `${PERSONALIZED_WORKFLOW_ROOT}/scripts/${RUNTIME_TASK_SCRIPT}`,
    );
  ...
  it("does NOT hash platform-dir files that are not in trackedPaths", () => {
    ...
    // Now: only paths written by the managed init flow (recorded via writeFile) make
    // it into the platform/root section of the manifest.
    fs.mkdirSync(path.join(tmpDir, PERSONALIZED_WORKFLOW_ROOT), {
      recursive: true,
    });
  ...
  it("excludes workspace and tasks directories", () => {
    fs.mkdirSync(path.join(tmpDir, PERSONALIZED_WORKFLOW_ROOT, "workspace"), {
      recursive: true,
    });
    fs.writeFileSync(
      path.join(tmpDir, PERSONALIZED_WORKFLOW_ROOT, "workspace", "data.md"),
      "user data",
    );
    fs.mkdirSync(path.join(tmpDir, PERSONALIZED_WORKFLOW_ROOT, "tasks"), {
      recursive: true,
    });
    fs.writeFileSync(
      path.join(tmpDir, PERSONALIZED_WORKFLOW_ROOT, "tasks", "task.json"),
      "{}",
    );
    ...
    expect(hashes).not.toHaveProperty(
      `${PERSONALIZED_WORKFLOW_ROOT}/workspace/data.md`,
    );
    expect(hashes).not.toHaveProperty(
      `${PERSONALIZED_WORKFLOW_ROOT}/tasks/task.json`,
    );
    expect(count).toBe(0);
  });
  ...
  it("excludes spec/ directory files from hashing", () => {
    fs.mkdirSync(
      path.join(tmpDir, PERSONALIZED_WORKFLOW_ROOT, "spec", "guides"),
      { recursive: true },
    );
    fs.mkdirSync(
      path.join(tmpDir, PERSONALIZED_WORKFLOW_ROOT, "spec", "frontend"),
      { recursive: true },
    );
    fs.mkdirSync(
      path.join(tmpDir, PERSONALIZED_WORKFLOW_ROOT, "spec", "backend"),
      { recursive: true },
    );
    fs.writeFileSync(
      path.join(
    ...
    expect(hashes).not.toHaveProperty(
      `${PERSONALIZED_WORKFLOW_ROOT}/spec/guides/index.md`,
    );
    expect(hashes).not.toHaveProperty(
      `${PERSONALIZED_WORKFLOW_ROOT}/spec/frontend/index.md`,
    );
    expect(hashes).not.toHaveProperty(
      `${PERSONALIZED_WORKFLOW_ROOT}/spec/backend/index.md`,
    );
    expect(count).toBe(0);
  });
  ...
  it("collectFiles returns POSIX-normalized paths (no backslashes)", () => {
    ...
    fs.mkdirSync(
      path.join(tmpDir, PERSONALIZED_WORKFLOW_ROOT, "scripts", "common"),
      {
        recursive: true,
      },
    );
    ...
    expect(hashes).toHaveProperty(
      `${PERSONALIZED_WORKFLOW_ROOT}/scripts/common/${RUNTIME_TASK_SCRIPT}`,
    );
  });
...
describe("cross-platform hash storage (POSIX keys + v2 schema)", () => {
  let tmpDir: string;
  const HASHES_REL = path.join(PERSONALIZED_WORKFLOW_ROOT, ".template-hashes.json");
  ...
  it("loadHashes returns {} for legacy flat-format file (no __version)", () => {
    ...
    const legacy = {
      [`${UNSUPPORTED_WORKFLOW_ROOT}\\config.yaml`]: "deadbeef",
  ...
  it("legacy file is discarded then initializeHashes regenerates v2", () => {
    ...
    fs.writeFileSync(
      path.join(tmpDir, HASHES_REL),
      JSON.stringify({ [`${UNSUPPORTED_WORKFLOW_ROOT}\\config.yaml`]: "deadbeef" }),
    );

    // Stage some real files to pick up.
    fs.mkdirSync(path.join(tmpDir, PERSONALIZED_SCRIPTS_DIR), {
      recursive: true,
    });
    fs.writeFileSync(
      path.join(tmpDir, PERSONALIZED_SCRIPTS_DIR, RUNTIME_TASK_SCRIPT),
    ...
    expect(parsed.hashes).not.toHaveProperty(`${UNSUPPORTED_WORKFLOW_ROOT}\\config.yaml`);
    expect(parsed.hashes).not.toHaveProperty(UNSUPPORTED_CONFIG_PATH);
    // Newly hashed file is present with POSIX key
    expect(parsed.hashes).toHaveProperty(
      `${PERSONALIZED_WORKFLOW_ROOT}/scripts/${RUNTIME_TASK_SCRIPT}`,
    );
  });
