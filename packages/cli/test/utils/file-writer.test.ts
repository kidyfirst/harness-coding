...
describe("ensureDir", () => {
  ...
  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), `${TMP_PREFIX}-ensure-`));
  });
...
describe("writeFile", () => {
  ...
  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), `${TMP_PREFIX}-write-`));
...
describe("write recording", () => {
  ...
  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), `${TMP_PREFIX}-record-`));
    setWriteMode("force");
  });
