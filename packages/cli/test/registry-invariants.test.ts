...
describe("registry internal consistency", () => {
  ...
  it("no configDir collides with the unsupported workflow root marker", () => {
    for (const id of PLATFORM_IDS) {
      expect(AI_TOOLS[id].configDir).not.toBe(UNSUPPORTED_WORKFLOW_ROOT);
    }
  });
  ...
  // agrees with the --claude/--codex/--gemini CLI flag users invoke.
...
describe("UserPromptSubmit hook wiring", () => {
  ...
  for (const { platform, path, event } of PLATFORM_HOOK_CONFIGS) {
    it(`${platform} hook config contains ${event} referencing inject-workflow-state.js`, async () => {
      ...
      expect(raw).toContain("inject-workflow-state.js");
    });
  }
