...
The task system is stored entirely under `tasks/` in the user project. Each task is a directory containing requirements, context, research, state, and relationship information.
...
The user sees a "current task," but active task state is stored per session.

```text
.runtime/sessions/<context-key>.json
```

`task.js start` writes the task path into the runtime session file for the current session. `task.js current --source` shows the current task and where it came from. Different AI windows can point to different tasks without overwriting each other.

If the platform or shell environment has no stable session identity, `task.js start` may be unable to set the active task. The AI should read the error, inspect the platform hook/session environment, and not fall back to a shared global pointer.
...
```jsonl
{"file": ".harness/spec/cli/backend/index.md", "reason": "Backend conventions"}
{"file": ".harness/tasks/04-28-example/research/api.md", "reason": "API research"}
```
...
```bash
node ./.harness/scripts/task.js create "<title>" --slug <slug>
node ./.harness/scripts/task.js start <task>
node ./.harness/scripts/task.js current --source
node ./.harness/scripts/task.js add-context <task> implement <file> <reason>
node ./.harness/scripts/task.js validate <task>
node ./.harness/scripts/task.js finish
node ./.harness/scripts/task.js archive <task>
```
...
| Need | Edit location |
| --- | --- |
| Change the default task template | `scripts/common/task_store.*` and task creation instructions. |
| Change status semantics | `workflow.md`, workflow-state hook logic, and task usage conventions. |
| Add task lifecycle actions | `hooks.after_*` in `config.yaml`. |
| Change context rules | Phase 1.3 in `workflow.md` and related platform agent/hook instructions. |
| Change archive policy | `scripts/common/task_store.*` or `task_utils.*`. |

These are local files in the user project. Do not default to editing CLI source code unless the user wants to contribute upstream.
