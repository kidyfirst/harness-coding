...
Skills and commands are textual entry points for user interaction with the generated workflow. Different platforms use different names, but their core purpose is the same: tell the AI how to enter the workflow when the user expresses a certain intent.
...
Workflow skills usually share one semantic set: brainstorm, before-dev, check, update-spec, break-loop. Multi-file built-in skills such as `harness-meta` use layered references.
...
They should not store task state; task state belongs in `tasks/` and `.runtime/`.
...
1. Keep entry files short; references carry long content.
2. Make trigger descriptions specific.
3. Keep the same semantics consistent across supported platforms.
4. Put project-specific capabilities in local skills.

If the user only wants local AI to know one more project rule, usually create a project-local skill or update `spec/` instead of changing a built-in workflow skill.
