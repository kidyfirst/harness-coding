...
4. Whether project rules already exist in `spec/`
...
| Team project conventions | Prefer `spec/` or a project-local skill. |
| Change workflow semantics | Synchronize `workflow.md`. |
...
- Which workflow files to read.
- Which scripts to run.
- How to report after completion.

If a command only repeats workflow rules, prefer making it reference/read `workflow.md` instead of maintaining a second copy of the flow.
...
```text
.claude/skills/project-harness-local/
└── SKILL.md
```

For multi-platform projects, add equivalent versions in each supported platform skill directory, or use `.agents/skills/` on platforms that support the shared layer.

## Notes

- Do not mix every platform's syntax into one file.
- Do not change only one platform entry point while claiming all supported platforms are covered.
- Do not hide long-term engineering conventions inside a command; write them to `spec/`.
