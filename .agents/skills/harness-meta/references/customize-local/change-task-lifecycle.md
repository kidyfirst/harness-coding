...
hooks:
  after_create:
    - "node .harness/scripts/hooks/my_sync.js create"
  after_start:
    - "node .harness/scripts/hooks/my_sync.js start"
  after_finish:
    - "node .harness/scripts/hooks/my_sync.js finish"
  after_archive:
    - "node .harness/scripts/hooks/my_sync.js archive"
```
...
1. Confirm the current task with `node ./.harness/scripts/task.js current --source`.
