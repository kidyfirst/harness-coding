...
description: "Bootstrap project-specific Harness Spec coding specs with a platform-neutral single-agent workflow. Use when creating or refreshing .harness/spec guidelines, analyzing a codebase with GitNexus, ABCoder, or source inspection, decomposing package/layer spec work, and writing real codebase-backed spec docs without placeholder text."
---

# Harness Spec Bootstrap

Use this skill to create or refresh `.harness/spec/` guidelines from the real codebase. One capable agent owns the full loop: analyze the repository, choose the spec boundaries, write the docs, and verify the result. The workflow does not depend on a specific host, CLI, or agent brand.

## Workflow

1. Confirm Harness Spec is initialized and inspect the current `.harness/spec/` tree.
...
- Do not leave placeholder text, empty headings, or copied boilerplate in `.harness/spec/`.

## Done Criteria

- `.harness/spec/` describes the project as it exists now.
