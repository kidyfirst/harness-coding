...
description: "Captures executable contracts and coding conventions into the personalized spec documents. Use when learning something valuable from debugging, implementing, or discussion that should be preserved for future sessions."
---
...
```bash
cat .harness/spec/<category>/<file>.md
```
...
```
Development Flow:
  Learn something → `update-spec` (Harness command) → Knowledge captured
       ↑                                  ↓
  `break-loop` (Harness command) ←──────────────────── Future sessions benefit
  (deep bug analysis)
```

- ``break-loop` (Harness command)` - Analyzes bugs deeply, often reveals spec updates needed
- ``update-spec` (Harness command)` - Actually makes the updates
- ``finish-work` (Harness command)` - Reminds you to check if specs need updates
