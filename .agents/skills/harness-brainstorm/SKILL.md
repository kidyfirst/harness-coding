...
Triggered from `start` (Harness command) when the user describes a development task, especially when:
...
```bash
TASK_DIR=$(node ./.harness/scripts/task.js create "brainstorm: <short goal>" --slug <auto>)
```

Use a slug without a date prefix. `task.js create` adds the `MM-DD-`
...
### Delegate to `harness-research` sub-agent (don't research inline)

For each research topic, **spawn a `harness-research` sub-agent via the Task tool** — don't do WebFetch / WebSearch / `gh api` inline in the main conversation.
...
> **Codex exception**: on Codex CLI, do NOT dispatch `harness-research` for research-first mode — do the research inline (WebFetch / WebSearch in the main session) and write findings to `{TASK_DIR}/research/<topic>.md` yourself. Reason: Codex `spawn_agent` runs sub-agents with isolated context, so the research sub-agent may not resolve the active task path via `task.js current` unless the dispatch prompt includes it explicitly. Inline research on Codex avoids this failure mode.

Agent type: `harness-research`
...
✅ Good:
```
Main agent: Task(subagent_type="harness-research",
...
Each `harness-research` sub-agent should:
...
```bash
# Create child tasks
CHILD1=$(node ./.harness/scripts/task.js create "Child task 1" --slug child1 --parent "$TASK_DIR")
CHILD2=$(node ./.harness/scripts/task.js create "Child task 2" --slug child2 --parent "$TASK_DIR")

# Or link existing tasks
node ./.harness/scripts/task.js add-subtask "$TASK_DIR" "$CHILD_DIR"
```
...
| Command | When to Use |
|---------|-------------|
| ``start` (Harness command)` | Entry point that triggers brainstorm |
| ``finish-work` (Harness command)` | After implementation is complete |
| ``update-spec` (Harness command)` | If new patterns emerge during work |
